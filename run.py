...
def refresh_stats():
    rows = get_stats()
    text = ""
    for subject, block, count in rows:
...
def handle_import():
    path, _ = QFileDialog.getOpenFileName(
        ui, "JSON test bazani tanlang", "", "JSON (*.json)"
    )
    if not path:
        return
...
def handle_clear():
    ...
    reply = QMessageBox.question(
        ui,
        "Tasdiqlash",
        f"{subject} [{block}] fanidagi BARCHA testlar o‘chiriladi.\nDavom etasizmi?",
...
def handle_generate():
    ...
    if a1 == a2:
        QMessageBox.warning(ui, "Xatolik", "Asosiy fanlar bir xil bo‘lmasin.")
        return
